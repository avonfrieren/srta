...
- `Source/RoomDeltas.cs` — phase 2. Delta ±X par room vs PB (`ThisRunTimes` vs `lastPbTimes`, comme la comparaison de fin de run de SpeedrunTool), deux modes via l'enum `DeltaMode` des settings : **Split** (temps total) ou **Room** (segment = différence des splits, clé précédente absente ⇒ 0 comme le `prevRoomTime` de SpeedrunTool), commutables par hotkey (`SwitchRoomDeltasMode`, cycle + popup comme le `SwitchRoomTimer` de SpeedrunTool). La ligne suit une **room de référence `deltaRoom`** et le texte est **recalculé à chaque frame** (affiché ssi run actuelle **et** PB ont un temps pour cette room) : `deltaRoom` passe à `roomNumber - 1` quand `RoomTimerData.roomNumber` bouge (polling après `orig` dans `On.Celeste.Level.Update` — srta enveloppe le hook de SpeedrunTool ⇒ données de la frame à jour ; couvre transitions, drapeaux, cœur/cassette, et se re-ancre sur reset/changement de type de timer), et passe à `NumberOfRooms` quand on scrolle le nombre de rooms — calqué sur SpeedrunTool dont `pbTimeKey = prefix + NumberOfRooms` re-cible l'affichage en live. Rendu : 3ᵉ ligne dessinée après `orig` de `SpeedrunTimerDisplay.Render`, avances de caractères via `SpeedrunTimerDisplay.GetTimeWidth` (public) pour ne pas publiciser Celeste. État statique enregistré via l'interop `SpeedrunTool.SaveLoad` (`RegisterStaticTypes`) — sauf le miroir de settings `lastNumberOfRooms`, les settings n'étant pas rollback par les save states. Masqué si le timer est caché ; visible aussi quand `IsCompleted` (en mode Split la valeur double alors la comparaison de fin de run de SpeedrunTool — nécessaire pour voir le delta en re-scrollant le nombre de rooms après un run, l'état étant alors `Completed`).
