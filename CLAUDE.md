...
- `Source/RoomDeltas.cs` — phase 2. Delta ±X par room vs PB (`ThisRunTimes` vs `lastPbTimes`, comme la comparaison de fin de run de SpeedrunTool), deux modes via l'enum `DeltaMode` des settings : **Split** (temps total) ou **Room** (segment = différence des splits, clé précédente absente ⇒ 0 comme le `prevRoomTime` de SpeedrunTool). Détection par **polling de `RoomTimerData.roomNumber`** après `orig` dans `On.Celeste.Level.Update` (srta enveloppe le hook de SpeedrunTool ⇒ données de la frame déjà à jour ; couvre transitions, drapeaux, cœur/cassette, et se resynchronise — delta effacé — sur reset/load state/changement de type de timer/changement de `NumberOfRooms` ou de mode). Rendu : 3ᵉ ligne dessinée après `orig` de `SpeedrunTimerDisplay.Render`, avances de caractères via `SpeedrunTimerDisplay.GetTimeWidth` (public) pour ne pas publiciser Celeste. État statique enregistré via l'interop `SpeedrunTool.SaveLoad` (`RegisterStaticTypes`) — sauf les miroirs de settings (`lastNumberOfRooms`, `lastDeltaMode`), les settings n'étant pas rollback par les save states. Masqué si `IsCompleted` (SpeedrunTool affiche alors sa propre comparaison) ou si le timer est caché.
